...
class CQLAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        batch_size_full, seq_len = batch['observations'].shape[0:2]
        ...
        # Select the first batch_size_full / num_eval_chunks_per_seq sequences
        batch_size = batch_size_full // self.config['num_eval_chunks_per_seq']
        batch = jax.tree_util.tree_map(
            lambda x: x[:batch_size],
            batch,
        )
        batch_select = jax.tree_util.tree_map(
            lambda x: x[:batch_size],
            batch_select,
        )
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        batch_size_full = batch['actions'].shape[0]
        ...
        # Select the first chunk in each sequence
        batch_select = jax.tree_util.tree_map(
            lambda x: x[:, 0:1],
            batch_select,
        )
        ...
        if self.config['actor_type'] in ('flow', 'distill-ddpg'):
            ...
            # BC flow loss.
            x_0 = jax.random.normal(x_rng, (batch_size_full, 1, self.config['action_chunk_dim']))
            x_1 = batch_select['action_chunks']
            t = jax.random.uniform(t_rng, (batch_size_full, 1, 1))
            x_t = (1 - t) * x_0 + t * x_1
            vel = x_1 - x_0
            ...
            if self.config["actor_type"] == "distill-ddpg":
                ...
                noises = jax.random.normal(noise_rng, (batch_size_full, 1, self.config['action_chunk_dim']))
                ...
                assert actor_actions.shape == (batch_size_full, 1, self.config['action_chunk_dim'])
