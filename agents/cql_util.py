...
def get_rectified_loss(
    ...
    # For both lower and upper bound losses, differences are valid if time diff
    # is positive, all chunks in [i,j] are valid, and all chunks in [i,j) are
    # nonterminals. The final chunk needs to be valid (inclusively) because in
    # the first case v_next is used from the end of the later chunk, and in the
    # second case q is evaluated on all the actions in the chunk, which must be
    # from the same contiguous trajectory.
